...
public class StreamAppl {
    public static void main(String[] args) {
        ...
        stream.map(movie -> movie.rating).distinct().forEach(System.out::println);
        System.out.println("================================");

        // 2. выводим первые 2 уникальные года фильмов
        Arrays.stream(movies)
                .map(movie -> movie.year).distinct().limit(2).forEach(System.out::println);
        System.out.println("================================");

        // 3. выводим только второй уникальный год из всех фильмов
        Arrays.stream(movies)
                .map(movie -> movie.year).distinct().skip(1).limit(1).forEach(System.out::println);
        System.out.println("================================");

        // 4. выводим на консоль только предпоследний элемент вашего массива.
        Arrays.stream(movies)
                .skip(movies.length -2).limit(1).forEach(System.out::println);
